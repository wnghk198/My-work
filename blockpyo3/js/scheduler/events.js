...
import { isProtected, stationCountsAt } from './core.js';
  ...
  const assignCore = (emp, st, fromIdx) => {
    ...
    for (let i = fromIdx; i < slotsMin.length && count < coreMaxSlots; i++) {
      ...
      if (cur === '' || cur === ST.OFF) {
        ...
        const maxOk =
          (st === ST.EXIT  && cnt.exit  < CONSTRAINTS.MAX_EXIT)  ||
          (st === ST.ELEV  && cnt.elev  < CONSTRAINTS.MAX_ELEV)  ||
          (st === ST.F3    && cnt.f3    < CONSTRAINTS.MAX_F3);
        if (!maxOk) break;
        row[i] = st; count++;
      } else break;
    }
