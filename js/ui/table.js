...
const LEGEND_STATIONS = [
  { st: ST.EXIT,      label: '2F출차',   css: 's-exit',      bg: '#3B6EF5' },
  { st: ST.ELEV,      label: '2F엘베',   css: 's-elev',      bg: '#8B5CF6' },
  { st: ST.F3,        label: '3F',        css: 's-3f',        bg: '#10B981' },
  { st: ST.CART2F,    label: '2F카트',   css: 's-cart2f',    bg: '#0EA5E9' },
  { st: ST.CART3F,    label: '3F카트',   css: 's-cart3f',    bg: '#14B8A6' },
  { st: ST.MEAL,      label: '식사',      css: 's-meal',      bg: '#F59E0B' },
  { st: ST.MTG,       label: '회의',      css: 's-mtg',       bg: '#EF4444' },
  { st: ST.REST,      label: '휴식',      css: 's-rest',      bg: '#475569' },
...
/** 블록 폭(슬롯 수)에 따른 표시 레이블 — { label, mins } */
function blockLabel(st, span, short) {
  const full = short(st);
  const mins = span * 15;
  if (!full || st === 'OFF' || st === '') return null;

  if (span === 1) {
    // 15분: 한 글자 약어만 (공간 없음)
    const abbr = { '출차':'차','엘베':'엘','3F':'3','2F카트':'카','3F카트':'F','식사':'밥','회의':'회','휴식':'쉬','오픈준비':'준' };
    return { label: abbr[full] ?? full.slice(0, 1), mins: null };
  }
  if (span === 2) {
    // 30분: 레이블 + 분 수 한 줄에
    return { label: full, mins };
  }
  // 45분+: 레이블 위 / 분 수 아래 두 줄
  return { label: full, mins };
}
    ...
    for (let ri = 0; ri < emps.length; ri++) {
      ...
      for (const blk of compressRow(schedule[e.id])) {
        ...
        if (blk.st && blk.st !== 'OFF' && blk.st !== '') {
          const lbl = blockLabel(blk.st, blk.span, short);
          if (lbl) {
            if (lbl.mins === null) {
              // 15분: 약어 한 줄
              td.textContent = lbl.label;
            } else {
              // 30분+: 스테이션명 + 분 수 두 줄
              td.innerHTML =
                `<span class="cell-name">${lbl.label}</span>` +
                `<span class="cell-mins">${lbl.mins}분</span>`;
            }
          }
