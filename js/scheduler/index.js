...
import { applyMeetings, applyOpenMeals, applyCloseMeals, applyOpenPrep, applyEarlyCoreCoverage, applyRestBreaks, applyPostMtgCoreSlot, applyPostMtgFixedSlots } from './events.js';
...
export function generateSchedule(input) {
  ...
  // ── Step 1: 고정 이벤트 ──────────────────────────────────────
  applyMeetings(ctx);
  applyPostMtgFixedSlots(ctx);   // 마감조 MTG 직후 2F카트→휴식 고정
  applyOpenMeals(ctx);
  applyCloseMeals(ctx);
  applyOpenPrep(ctx);
  ...
  // forceCoreExperience 스왑으로 점유자가 코어미경험이 된 경우 복구
  fixFragmentBlocks(ctx);
  enforceCart2fMax(ctx);
  ensureCoreExperience(ctx);
  fixFragmentBlocks(ctx);
  ...
  applyPostMtgFixedSlots(ctx);   // 마감조 MTG 직후 2F카트→휴식 재적용 (수리 패스 후)
  repairCoverage(ctx);

  // ── Step 7 마무리: 짧은 코어 블록 최종 정리 ─────────────────
  for(let _fin = 0; _fin < 4; _fin++){
    fixFragmentBlocks(ctx);
    enforceCart2fMax(ctx);
    enforceCart3fMax(ctx);
    repairCoverage(ctx);
  ...
  // ── 최후 강제 변환: 커버리지 무시하고 남은 coreMin 미달 블록 제거 ──
  // fixFragmentBlocks의 coverBreak 체크에도 막힌 블록을 CART2F로 강제 변환
  // (커버리지 결핍이 발생하더라도 블록길이 규칙 우선)
  for(const e of employees){
    const row    = schedule[e.id];
    const actRow = active[e.id];
    let i = 0;
    while(i < row.length){
      ...
      if(bE - bS < ctx.coreMinSlots){
        ...
        // 가장 긴 인접 코어 블록에 흡수 (coreMax 초과 않으면)
        if(prevSt && isCore(prevSt) && prevLen+(bE-bS) <= ctx.coreMaxSlots){
          for(let j=bS;j<bE;j++) row[j]=prevSt; merged=true;
        } else if(nextSt && isCore(nextSt) && nextLen+(bE-bS) <= ctx.coreMaxSlots){
          for(let j=bS;j<bE;j++) row[j]=nextSt; merged=true;
        }
        if(!merged){
          // 흡수 불가 → CART2F 강제 전환
  ...
  enforceCart2fMax(ctx);
  enforceCart3fMax(ctx);
  enforceMaxBlockLength(ctx);  // coreMax 초과 블록 최종 강제 분할
  enforceCart2fMax(ctx);
  enforceCart3fMax(ctx);

  // ── 최후 코어미경험 보정: 최종 상태에서 한 번 더 시도 ─────────
  // 파이프라인 후반 상태가 forceCoreExperience가 처음 실행될 때와 달라
  // "shouldWork" 케이스가 최후에 해결 가능해진 경우를 처리
  forceCoreExperience(ctx);
  fixFragmentBlocks(ctx);
  enforceCart2fMax(ctx);
  ensureCoreExperience(ctx);   // 스왑으로 점유자 코어미경험 복구
  repairCoverage(ctx);         // Pass A 직접 배정이 만든 커버리지 결핍 복구
  enforceCart3fMax(ctx);
