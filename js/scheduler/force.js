    ...
    for(const nSt of [ST.EXIT,ST.ELEV,ST.F3]){
      ...
      if(!placed){
        ...
        for(let si=0;si<slotsMin.length&&!placed;si++){
          ...
          // 점유자 잔류 확인 (스왑 후 nSt 경험 유지)
          const occTotal=oRow.filter((s,i)=>oActRow[i]&&s===nSt).length;
          if(occTotal-swapLen<coreMinSlots&&occTotal-swapLen<0)continue; // 점유자 완전 박탈 방지
          if(occTotal<=swapLen)continue; // 점유자 경험 완전 박탈 방지

          const snapE=row.slice();const snapO=oRow.slice();
          const eSt=row[si];
          for(let j=si;j<si+swapLen;j++){row[j]=nSt;oRow[j-(si-ibS)+(ibS)]=eSt;}
          // 실제로는 같은 위치 교환 (si..si+swapLen)
          for(let j=0;j<swapLen;j++){row[si+j]=nSt;oRow[si+j]=eSt;}

          // 방어 검증
          let ok=true;
          for(let j=si;j<si+swapLen&&ok;j++){
