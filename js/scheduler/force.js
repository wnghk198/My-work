  ...
  for(const e of employees){
    ...
    for(const nSt of [ST.EXIT,ST.ELEV,ST.F3]){
      ...
      for(let si=0;si<slotsMin.length;si++){
        ...
        const cnt=stationCountsAt(schedule,employees,active,si);
        const maxReached=(nSt===ST.EXIT&&cnt.exit>=dynMaxAt(ST.EXIT,si,employees,active))||
          (nSt===ST.ELEV&&cnt.elev>=dynMaxAt(ST.ELEV,si,employees,active))||
          (nSt===ST.F3&&cnt.f3>=dynMaxAt(ST.F3,si,employees,active));
        ...
        if(maxReached){
          ...
          // 방어 검증 (MAX, 커버리지)
          const cnt2=stationCountsAt(schedule,employees,active,si);
          let ok=(cnt2.exit<=dynMaxAt(ST.EXIT,si,employees,active)&&cnt2.elev<=dynMaxAt(ST.ELEV,si,employees,active)&&cnt2.f3<=dynMaxAt(ST.F3,si,employees,active));
  ...
  for(let iter=0;iter<50;iter++){
    ...
    for(const d of defs){
      ...
      for(const nSt of needs){
        ...
        if(nSt===ST.EXIT&&cnt.exit>=dynMaxAt(ST.EXIT,si,employees,active))continue;
        if(nSt===ST.ELEV&&cnt.elev>=dynMaxAt(ST.ELEV,si,employees,active))continue;
        if(nSt===ST.F3&&cnt.f3>=dynMaxAt(ST.F3,si,employees,active))continue;
