...
import { minToTime, minsToSlots, slotIndexOf } from '../utils.js';
...
export function recolorBlock(ctx, empId, tIdx, targetSt) {
  ...
  // 마감 야간 면제
  if (slotsMin[tIdx] >= CONSTRAINTS.CLOSE_LATE_MIN) { row[tIdx] = targetSt; return true; }
  ...
  const maxNew     = Math.max(0, coreMaxSlots - beforeRun);
  ...
  const _maxOkAt = (si) => {
    ...
    if (targetSt === ST.EXIT  && cc.exit  >= CONSTRAINTS.MAX_EXIT)  return false;
    if (targetSt === ST.ELEV  && cc.elev  >= CONSTRAINTS.MAX_ELEV)  return false;
    return true;
  };
  ...
  while (need > 0 && ext < row.length) {
    ...
    if (beforeRun + (eE - bS) + 1 > coreMaxSlots) break;
  ...
  // 마감조 안전회의 직후 1슬롯 예외
  const ctx_e = ctx.employees?.find(e => e.id === empId);
  if (ctx_e?.group === 'CLOSE' && eE - eS === 1 && eS > 0 && row[eS - 1] === ST.MTG) {
