...
export function applyMidMeals(ctx) {
  ...
  // 16:40 이후 가장 가까운 유효 슬롯 인덱스 (findIndex로 정확한 매칭 없어도 안전하게 처리)
  ...
  for (const e of midEmps) {
    ...
    if (FIXED_MEAL_STARTS.has(e.startMin)) {
      // Bug Fix 1: 16:40은 슬롯 그리드에 없으므로 findIndex로 가장 가까운 슬롯 사용
      if (fixedMealIdx >= 0) {
        mealStart = findNearestSlot(row, actRow, fixedMealIdx, eS, eE);
      }
    } else {
      // Bug Fix 2: 11:45 미만 + 그 외(13:30, 13:50 등) 모두 +3시간 규칙 적용
