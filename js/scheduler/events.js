...
// ── 마감조 안전회의 직후 고정 배치 ─────────────────────────────
// 마감조 직원의 안전회의(MTG) 종료 직후 2슬롯을 고정:
//   +0슬롯(15분): 2F카트(CART2F)
//   +1슬롯(15분): 휴식(REST)
// 이미 보호된 슬롯(MEAL 등)이면 덮어쓰지 않음
export function applyPostMtgFixedSlots(ctx) {
  ...
  for (const e of close) {
    ...
    for (let i = 0; i < slotsMin.length - 1; i++) {
      ...
      if (row[i] !== ST.MTG) continue;
      // MTG 블록 끝 탐색 (연속 MTG 슬롯 이후)
      if (i + 1 < slotsMin.length && row[i + 1] === ST.MTG) continue; // 아직 MTG 블록 내부
      // i = MTG 마지막 슬롯 → i+1, i+2 고정
      const s1 = i + 1;
      const s2 = i + 2;
      if (s1 < slotsMin.length && actRow[s1] && row[s1] !== ST.MEAL)
        row[s1] = ST.CART2F;
      if (s2 < slotsMin.length && actRow[s2] && row[s2] !== ST.MEAL)
        row[s2] = ST.REST;
    }
  }
}

// applyPostMtgCoreSlot: 이전 호환성을 위해 no-op 유지
export function applyPostMtgCoreSlot(_ctx) { /* no-op */ }
