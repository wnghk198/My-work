    ...
    for(const d of defs){
      ...
      for(const nSt of needs){
        ...
        const canFormBlock=(e)=>{
          ...
          // 실제로 nSt로 재색칠 가능한 연속 슬롯 수 측정
          // 보호 슬롯·OFF·오픈준비 + 이미 다른 코어가 커버리지 제공 중인 슬롯은 불가
          const curSt=row[d.idx];
          // d.idx 와 같은 스테이션 블록 범위를 기준으로 연속성 측정
          let cnt=0;
          // 현재 블록 방향 탐색 (같은 curSt 연속 범위)
          let bS=d.idx,bE=d.idx+1;
          while(bS>0&&actRow[bS-1]&&!isProtected(row[bS-1])&&row[bS-1]!==ST.OFF&&row[bS-1]!==ST.OPEN_PREP)bS--;
          while(bE<row.length&&actRow[bE]&&!isProtected(row[bE])&&row[bE]!==ST.OFF&&row[bE]!==ST.OPEN_PREP)bE++;
          cnt=bE-bS;
          return cnt>=minBlk;
        };
