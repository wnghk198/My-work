...
import { isCore, isProtected, stationCountsAt, computeCoverageDeficits, getFairnessCost, recolorBlock } from './core.js';
    ...
    for(const d of defs){
      ...
      for(const nSt of needs){
        ...
        for(const e of employees){
          ...
          if(nSt===ST.EXIT){if(cntAtD.exit>=CONSTRAINTS.MAX_EXIT)continue;}
          else if(nSt===ST.ELEV){if(cntAtD.elev>=CONSTRAINTS.MAX_ELEV)continue;}
        ...
        const maxOk=(nSt===ST.EXIT&&cntCheck.exit<CONSTRAINTS.MAX_EXIT)||(nSt===ST.ELEV&&cntCheck.elev<CONSTRAINTS.MAX_ELEV)||(nSt===ST.F3&&cntCheck.f3<CONSTRAINTS.MAX_F3);
        if(!maxOk)continue;
        // recolor 후 어떤 슬롯이든 MAX 초과하면 롤백
        const snapshot=schedule[winner.id].slice();
        recolorBlock(ctx,winner.id,d.idx,nSt);
        // 롤백 검사: winner가 배정된 범위에서 MAX 위반 확인
        let violated=false;
        for(let si=0;si<slotsMin.length;si++){
          ...
          if((nSt===ST.EXIT&&cc.exit>CONSTRAINTS.MAX_EXIT)||(nSt===ST.ELEV&&cc.elev>CONSTRAINTS.MAX_ELEV)){violated=true;break;}
        }
  ...
  for(const e of employees){
    ...
    for(const nSt of missing){
      ...
      for(const nb of nc){
        ...
        const maxOk=(nSt===ST.EXIT&&cnt.exit<CONSTRAINTS.MAX_EXIT)||(nSt===ST.ELEV&&cnt.elev<CONSTRAINTS.MAX_ELEV)||(nSt===ST.F3);
        if(!maxOk)continue;
        // 인접 같은 코어 합산 coreMax 초과 방지
        ...
        for(let si=nb.start;si<replEnd;si++){
          ...
          if((nSt===ST.EXIT&&cc.exit>=CONSTRAINTS.MAX_EXIT)||(nSt===ST.ELEV&&cc.elev>=CONSTRAINTS.MAX_ELEV)){blockViolated=true;break;}
        }
...
export function ensureCoreExperience(ctx){
  ...
  const maxOf=(st)=>st===ST.EXIT?CONSTRAINTS.MAX_EXIT:st===ST.ELEV?CONSTRAINTS.MAX_ELEV:CONSTRAINTS.MAX_F3;
      ...
      for(const b of candBlocks){
        for(let ws=b.start;ws+coreMinSlots<=b.end;ws++){
          ...
          for(let si=ws;si<cap;si++){
            const cc=stationCountsAt(schedule,employees,active,si);
            if(cntOf(cc,nSt)>=maxOf(nSt))break;
            we=si+1;
          }
      ...
      for(const b of candBlocks){
        ...
        while(wi+coreMinSlots<=b.end){
          ...
          for(let si=swapS;si<swapS+swapLen&&ok;si++){
            const cc=stationCountsAt(schedule,employees,active,si);
            if(cc.exit>CONSTRAINTS.MAX_EXIT)ok=false;
            if(cc.elev>CONSTRAINTS.MAX_ELEV)ok=false;
            if(cc.f3>CONSTRAINTS.MAX_F3)ok=false;
      ...
      for(const b of candBlocks){
        ...
        while(wi<b.end){
          ...
          for(let si=swapS;si<swapS+useLen&&ok3;si++){
            const cc=stationCountsAt(schedule,employees,active,si);
            if(cc.exit>CONSTRAINTS.MAX_EXIT)ok3=false;
            if(cc.elev>CONSTRAINTS.MAX_ELEV)ok3=false;
            if(cc.f3>CONSTRAINTS.MAX_F3)ok3=false;
          }
