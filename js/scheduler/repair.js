    ...
    for(const d of defs){
      ...
      for(const nSt of needs){
        ...
        const canFormBlock=(e)=>{
          ...
          // d.idx 슬롯과 같은 스테이션의 연속 블록 범위만 측정
          // 다른 스테이션 블록까지 확장하면 실제 교체 가능 범위를 과대 계산하는 버그 수정
          const curSt=row[d.idx];
          let bS=d.idx,bE=d.idx+1;
          while(bS>0&&actRow[bS-1]&&row[bS-1]===curSt&&!isProtected(row[bS-1])&&row[bS-1]!==ST.OFF&&row[bS-1]!==ST.OPEN_PREP)bS--;
          while(bE<row.length&&actRow[bE]&&row[bE]===curSt&&!isProtected(row[bE])&&row[bE]!==ST.OFF&&row[bE]!==ST.OPEN_PREP)bE++;
          return (bE-bS)>=minBlk;
        };
        ...
        // coreCount 해당 직원 전체 재계산 (단방향 증가 버그 수정)
        if(coreCount && coreCount[winner.id]){
