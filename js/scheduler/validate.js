...
export function validate(ctx) {
  ...
  for (const e of employees) {
    ...
    for (const b of blocks) {
      ...
      const isPostMtgExc   = isClose && len === 1 && b.start > 0 && row[b.start - 1] === ST.MTG;
      const isForcedCovExc = len < coreMinSlots && isCore(b.st);

      if (isCore(b.st) && len < coreMinSlots && !isPostMtgExc && !isForcedCovExc)
