...
function initEqualWorkBlockTargets(workers, globalWarnings) {
  ...
  // 6시간/8시간 그룹은 서로 다른 기준을 적용한다(그룹 간 차이는 허용).
  // 같은 그룹 내부에서는 "지나치게" 차이나지 않도록,
  //  - 가능하면(교집합 존재) 동일 목표(편차 0)
  //  - 불가능하면 각자의 가능 범위 내에서 편차가 최소가 되도록 목표를 분배
  [6, 8].forEach(function(h) {
    ...
    group.forEach(function(w) {
      ...
      const pre = (w.segments || []).filter(function(seg){ return seg && seg.type === 'work'; }).length;
      w._minPossibleBlocks = Math.max(6, pre + (stats.minSeg || 0));
      w._maxPossibleBlocks = Math.max(6, pre + (stats.maxSeg || 0));
    });

    const groupMin = Math.max.apply(null, group.map(function(w) { return w._minPossibleBlocks || 0; }));
    const groupMax = Math.min.apply(null, group.map(function(w) { return w._maxPossibleBlocks || 0; }));

    // 1) 교집합이 있으면: 그룹 내부 동일 목표(=편차 0)
    if (groupMin <= groupMax) {
      const target = groupMin; // 블록 수를 불필요하게 늘리지 않도록 최소값 사용
      group.forEach(function(w) {
        w._targetWorkBlocks = target;
        w._workBlocksAssigned = (w.segments || []).filter(function(seg){ return seg && seg.type === 'work'; }).length;
      });
      return;
    }

    // 2) 교집합이 없으면: 가능한 범위 내에서 편차(최대-최소)가 최소가 되도록 목표를 분배
    // 최소 편차 창(window)의 양끝은 [groupMax, groupMin]
    const low = groupMax;
    const high = groupMin;
    ...
    const assigned = [];
    group.forEach(function(w) {
      const t = Math.min(w._maxPossibleBlocks, Math.max(w._minPossibleBlocks, center));
      w._targetWorkBlocks = t;
      w._workBlocksAssigned = (w.segments || []).filter(function(seg){ return seg && seg.type === 'work'; }).length;
      assigned.push(t);
    });

    const minT = Math.min.apply(null, assigned);
    const maxT = Math.max.apply(null, assigned);
    if (globalWarnings) {
      globalWarnings.push('주의: ' + h + '시간 그룹에서 모두 같은 배치 횟수를 만들 수 없어, 가능한 범위에서 편차를 최소화했습니다. (' + minT + '~' + maxT + '회)');
    }
  });
}
...
// ===== 코어 근무지(2F 출차/2F 엘베/3F) 1회 이상 보장(하드 제약) =====
// 모든 근무자는 출근한 하루 동안 아래 코어 근무지를 최소 1회 이상 수행해야 한다.
// - 2F 엘베 (60분)
// - 2F 출차 (60분)
// - 3F (60분)
// 이 조건은 '점수 우선'이 아니라, 가능한 후보가 있는 한 "미수행 코어 근무지"만 후보로 제한하는 하드 제약으로 적용한다.

const CORE_REQUIRED_WPS = ['2F 엘베', '2F 출차', '3F'];
...
function initWorkerAllWpRequirement(worker) {
  if (!worker) return;
  worker.requiredWps = new Set(CORE_REQUIRED_WPS);
  const requiredMin = CORE_REQUIRED_WPS.reduce(function(sum, wp) { return sum + (MIN_ONCE_BLOCK_BY_WP[wp] || 60); }, 0);
  worker._coreRequiredMin = requiredMin;

  const expected = computeExpectedWorkMin(worker);
  worker._coreImpossible = expected < requiredMin;
}
...
function preallocateCoreRequiredBlocks(workers, coverage, rng) {
  // v112: 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상을 "후보 우선"이 아니라
  // 사전 예약(Pre-allocation)으로 강제한다.
  // - 각 근무자별로 60분 코어 블록 3개를 먼저 배치(가능한 구간만)
  // - 배치 시 정원(WP_CAPS)을 지키고, 코어 결손 슬롯(0명)을 우선적으로 메운다.
  const list = (workers || []).slice();

  const core = CORE_REQUIRED_WPS.slice();
  const minCorePerSlot = { '2F 엘베': 1, '2F 출차': 1, '3F': 1 };
  ...
  function scoreSlot(wp, s, e) {
    // 커버리지 결손(최소치 미달)을 얼마나 메우는지 점수화
    let score = 0;
    for (const t of slotRange(s, e)) {
      const c = getCounts(coverage, t);
      const need = minCorePerSlot[wp] || 0;
      const cur = c[wp] || 0;
      if (cur < need) score += (need - cur);
    }
    return score;
  }

  function enumerateCandidates(worker, wp) {
    const out = [];
    const dur = 60;
    // 5분 단위 탐색
    for (let s = worker.startMin; s + dur <= worker.endMin; s += 5) {
      const e = s + dur;
      if (!preservesMinWorkGap(worker, s, e)) continue;
      if (!canAssignToWp(wp, s, e, coverage)) continue;
      out.push({ s, e, sc: scoreSlot(wp, s, e) });
    }
    // 결손을 더 메우는 슬롯 우선
    out.sort((a, b) => (b.sc - a.sc) || (a.s - b.s));
    return out;
  }

  list.forEach(function(worker) {
    if (!worker || worker._coreImpossible) return;
    // 이미 배치된 코어는 스킵
    const done = new Set();
    (worker.segments || []).forEach(function(seg) {
      if (seg && seg.type === 'work' && CORE_REQUIRED_WPS.indexOf(seg.label) !== -1) done.add(seg.label);
    });

    for (const wp of core) {
      if (done.has(wp)) continue;

      const candidates = enumerateCandidates(worker, wp);
      if (!candidates.length) {
        worker.warnings.push('코어 사전배치 실패: ' + wp + ' (60분 구간 부족)');
        continue;
      }

      // 상위 후보군 중 랜덤(동률 분산)
      const topScore = candidates[0].sc;
      const top = candidates.filter(x => x.sc === topScore).slice(0, 10);
      const chosen = pick(top) || candidates[0];

      const seg = addSegment(worker, chosen.s, chosen.e, 'work', wp);
      if (seg) seg.locked = true;
      assignCoverage(worker, wp, chosen.s, chosen.e, coverage);
      done.add(wp);
    }
  });
}
...
// ===== 코어 커버리지(시간대별) 우선 채우기: 하드 필터 =====
// 문제: 점수 기반(soft) 우선순위만으로는 특정 시간대에 2F 엘베/2F 출차/3F가 0명이 되는 케이스가 발생할 수 있음.
// 해결: 해당 시간대에 코어 근무지 인원이 최소치 미달이면, 가능한 후보가 있는 한 그 코어 근무지로 후보를 제한한다.
// (단, 물리적으로 불가능한 경우엔 기존 후보를 유지)

const CORE_COVERAGE_MIN = { '2F 엘베': 1, '2F 출차': 1, '3F': 1 };

function coreNeedSlots(coverage, wp, start, end) {
  const min = CORE_COVERAGE_MIN[wp] || 0;
  if (!min) return 0;
  let need = 0;
  for (const t of slotRange(start, end)) {
    const c = getCounts(coverage, t);
    if ((c[wp] || 0) < min) need += 1;
  }
  return need;
}

function applyCoreCoverageHardFilter(candidates, coverage, start) {
  if (!candidates || candidates.length <= 1) return candidates;
  let best = 0;
  const scores = new Map();

  for (const c of candidates) {
    if (!CORE_COVERAGE_MIN[c.wp]) continue;
    const w = (c.wp === '2F 엘베' || c.wp === '2F 출차') ? 100 : 50;
    const need = coreNeedSlots(coverage, c.wp, start, start + c.duration);
    const s = need * w;
    scores.set(c, s);
    if (s > best) best = s;
  }
  ...
  const filtered = candidates.filter(c => scores.get(c) === best);
  return filtered.length ? filtered : candidates;
}

function applyCoreCoverageHardFilterToWps(wpList, coverage, start, end) {
  if (!wpList || wpList.length <= 1) return wpList;
  let best = 0;
  const scores = new Map();

  for (const wp of wpList) {
    if (!CORE_COVERAGE_MIN[wp]) continue;
    const w = (wp === '2F 엘베' || wp === '2F 출차') ? 100 : 50;
    const need = coreNeedSlots(coverage, wp, start, end);
    const s = need * w;
    scores.set(wp, s);
    if (s > best) best = s;
  }
  if (best <= 0) return wpList;
  const filtered = wpList.filter(wp => scores.get(wp) === best);
  return filtered.length ? filtered : wpList;
}
...
function applyOpenKickoffFilter(candidates, worker, start) {
  ...
  if (stage < 2) {
    ...
    // 코어 미수행이 모두 고정(60분) 근무지인 경우가 많으므로,
    // stage 0~1에서도 가능한 한 고정(60분) 코어 근무지부터 채운다.
...
if (stage === 2) {
  ...
  let CURRENT_PRESENT_COVERAGE = null;
  let CURRENT_NON_WORK_COVERAGE = null;
  ...
  function overlapsMealBreakWindow(start, end, presentCoverage, nonWorkCoverage) {
    // v112: 기존 "식사/휴식 시간에는 카트 금지" 규칙을 전역 고정이 아니라,
    // 실제로 카트가 '불필요한' 시간대(=비카트 정원으로 커버 가능한 경우)에만 적용하도록 완화한다.
    // - nonWork(식사/휴식/회의/준비 등) 인원이 1명 이상 존재하는 슬롯에서만 판단
    // - active(출근-비근무)가 NON_CART_CAP_TOTAL 이하이면 카트 금지(비카트로 충분히 커버 가능)
    // - active가 더 많아 비카트 정원으로 수용 불가하면 카트 허용(현실적 커버리지 확보)
    const pRef = presentCoverage || CURRENT_PRESENT_COVERAGE || {};
    const nRef = nonWorkCoverage || CURRENT_NON_WORK_COVERAGE || {};
    for (const t of slotRange(start, end)) {
      const non = getSimpleCount(nRef, t);
      if (non <= 0) continue;
      const present = getSimpleCount(pRef, t);
      const active = present - non;
      if (active <= NON_CART_CAP_TOTAL) return true;
    }
    return false;
  }
...
function insertOpenShiftFixedBlocks(worker) {
  ...
  function validateAllWorkplacesAtLeastOnce(workers) {
    const issues = [];
    (workers || []).forEach(function(w) {
      const missing = CORE_REQUIRED_WPS.filter(function(wp) {
        return !(w.placeCounts && (w.placeCounts[wp] || 0) > 0);
      });
      if (missing.length) issues.push(w.name + ': ' + missing.join(', '));
    });
    return issues;
  }
...
function rebalanceDailyJobRepetitionBySwaps(workers, rng) {
  ...
  // ===== 코어 근무지(2F 출차/2F 엘베/3F) 1회 이상 보장: 스왑(교환) 후처리 =====
// 가능한 범위에서 "같은 시간 구간(start~end)"의 근무지 라벨만 교환하여,
// 커버리지(시간대별 인원수)는 유지하면서 각 근무자가 코어 근무지(2F 엘베/2F 출차/3F)를 최소 1회 이상 수행하도록 보정한다.
function ensureAllWorkplacesAtLeastOnceBySwaps(workers, rng) {
  ...
  function missingFromCounts(counts) {
    const miss = new Set();
    CORE_REQUIRED_WPS.forEach(function(wp) { if ((counts[wp] || 0) <= 0) miss.add(wp); });
    return miss;
  }
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    const segAChoices = stA.segs.filter(function(seg) {
      const wp = seg.label;
      if (CORE_REQUIRED_WPS.indexOf(wp) === -1) return true; // 비코어는 0회가 되어도 허용
      return (stA.counts[wp] || 0) >= 2; // 코어는 최소 1회 유지
    });
    ...
    for (let attempt = 0; attempt < Math.min(36, fallbackSegAChoices.length); attempt++) {
      ...
      if (CORE_REQUIRED_WPS.indexOf(wpHave) !== -1 && (stA.counts[wpHave] || 0) <= 1) continue; // 코어 근무지는 0회가 되면 안 됨
...
// ===== 공평성(근무지 분배 균형) 점검/자동 보정 =====
// - 시간대별 커버리지(인원수)는 그대로 유지하면서(=같은 시간구간 start-end끼리만 스왑)
//   개인별 근무지 편중을 줄인다.
// - "코어 근무지 최소 1회" 조건이 깨지지 않도록, 각 근무자가 해당 근무지를 2회 이상 수행한 경우에만
//   그 근무지를 다른 것으로 스왑하여 0회가 되는 것을 방지한다.
// - 6h/8h 그룹 내부에서만 균형을 맞춘다(요구 조건: 시간 그룹별 동일/공평).
function assessFairWorkplaceDistribution(workers) {
  ...
  function buildMinutes(w) {
    const minutes = Object.fromEntries(WORKPLACES.map(function(p) { return [p, 0]; }));
    (w.segments || []).forEach(function(seg) {
      if (seg.type !== 'work' || !seg.label) return;
      if (minutes[seg.label] == null) return;
      minutes[seg.label] += (seg.end - seg.start);
    });
    return minutes;
  }

  function stats(values) {
    if (!values.length) return { avg: 0, min: 0, max: 0, range: 0, stdev: 0 };
    const avg = values.reduce((a, b) => a + b, 0) / values.length;
    const min = Math.min.apply(null, values);
    const max = Math.max.apply(null, values);
    const v = values.reduce((s, x) => s + Math.pow(x - avg, 2), 0) / values.length;
    return { avg, min, max, range: max - min, stdev: Math.sqrt(v) };
  }

  const report = { grade: '양호', issues: [], groups: [] };

  [8, 6].forEach(function(h) {
    const group = byHours.get(h) || [];
    if (!group.length) return;

    const rows = group.map(function(w) {
      return { id: w.id, name: w.name, minutes: buildMinutes(w) };
    });

    const wpStats = {};
    WORKPLACES.forEach(function(wp) {
      const vals = rows.map(r => r.minutes[wp] || 0);
      wpStats[wp] = stats(vals);
    });

    // 임계값: 60분 블록 중심 업무는 범위 120분 초과면 "주의", 30분 블록 중심은 90분 초과면 "주의",
    // 보조는 제한적이므로 60분 초과면 "주의".
    const thresholds = {
      '2F 엘베': 120,
      '2F 출차': 120,
      '3F': 120,
    ...
    const groupIssues = [];
    WORKPLACES.forEach(function(wp) {
      const thr = thresholds[wp] != null ? thresholds[wp] : 120;
      if ((wpStats[wp].range || 0) > thr) {
        groupIssues.push(h + '시간 ' + wp + ' 편차 범위 ' + Math.round(wpStats[wp].range) + '분 (최소 ' + Math.round(wpStats[wp].min) + ' / 최대 ' + Math.round(wpStats[wp].max) + ')');
      }
    });

    if (groupIssues.length) {
      report.grade = '주의';
      report.issues = report.issues.concat(groupIssues);
    }

    report.groups.push({ hours: h, count: group.length, wpStats: wpStats, rows: rows });
  });

  return report;
}

function rebalanceFairWorkplaceDistributionBySwaps(workers, rng) {
  const list = Array.isArray(workers) ? workers.slice() : [];
  const fixes = [];

  function pick(arr) { return rng && typeof rng.pick === 'function' ? rng.pick(arr) : (arr && arr.length ? arr[Math.floor(Math.random() * arr.length)] : null); }

  function getWorkSegs(w) {
    return (w.segments || []).filter(function(s) { return s.type === 'work' && s.label; })
      .slice().sort(function(a, b) { return a.start - b.start; });
  }
  ...
  function canWorkerTake(w, counts, seg, newWp) {
    ...
    // 첫 work 세그먼트는 스왑하지 않는다(첫 배정 정책 유지)
    const workSegs = getWorkSegs(w);
    if (workSegs.length && seg === workSegs[0]) return false;

    if (isFixed60(newWp) && dur !== 60) return false;

    // 보조는 30분만 허용
    if (newWp === ASSIST_WP && dur !== 30) return false;

    // 보조 최대 2회
    if (newWp === ASSIST_WP) {
      const cur = counts[ASSIST_WP] || 0;
      const next = cur + (seg.label === ASSIST_WP ? 0 : 1);
      if (next > MAX_ASSIST_PER_WORKER) return false;
    }

    // 코어 근무지(2F 엘베/2F 출차/3F)는 0회가 되면 안 됨
    if (CORE_REQUIRED_WPS.indexOf(seg.label) !== -1 && (counts[seg.label] || 0) <= 1) return false;
  ...
  function buildState(group) {
    const state = new Map();
    const totals = Object.fromEntries(WORKPLACES.map(function(p) { return [p, 0]; }));
    group.forEach(function(w) {
      const segs = getWorkSegs(w);
      const minutes = Object.fromEntries(WORKPLACES.map(function(p) { return [p, 0]; }));
      const counts = Object.fromEntries(WORKPLACES.map(function(p) { return [p, 0]; }));
      segs.forEach(function(seg) {
        minutes[seg.label] = (minutes[seg.label] || 0) + (seg.end - seg.start);
        counts[seg.label] = (counts[seg.label] || 0) + 1;
      });
      WORKPLACES.forEach(function(p) { totals[p] += (minutes[p] || 0); });
      state.set(w.id, { worker: w, segs: segs, minutes: minutes, counts: counts });
    });
    const n = Math.max(1, group.length);
    const targets = Object.fromEntries(WORKPLACES.map(function(p) { return [p, totals[p] / n]; }));
    return { state, targets };
  }

  const weights = Object.fromEntries(WORKPLACES.map(function(p) { return [p, 1]; }));
  weights[ASSIST_WP] = 0.7;

  function err(min, wp, targets) {
    const t = targets[wp] || 0;
    const w = weights[wp] || 1;
    const d = (min || 0) - t;
    return w * d * d;
  }

  function deltaForSwap(stA, stB, wpA, wpB, d, targets) {
    // A: wpA 감소, wpB 증가
    const aOld = err(stA.minutes[wpA], wpA, targets) + err(stA.minutes[wpB], wpB, targets);
    const aNew = err((stA.minutes[wpA] || 0) - d, wpA, targets) + err((stA.minutes[wpB] || 0) + d, wpB, targets);
    // B: wpB 감소, wpA 증가
    const bOld = err(stB.minutes[wpB], wpB, targets) + err(stB.minutes[wpA], wpA, targets);
    const bNew = err((stB.minutes[wpB] || 0) - d, wpB, targets) + err((stB.minutes[wpA] || 0) + d, wpA, targets);
    return (aNew + bNew) - (aOld + bOld);
  }

  function updateAfterSwap(stA, stB, wpA, wpB, d) {
    // A loses wpA, gains wpB
    stA.minutes[wpA] -= d;
    stA.minutes[wpB] = (stA.minutes[wpB] || 0) + d;
    stA.counts[wpA] -= 1;
    stA.counts[wpB] = (stA.counts[wpB] || 0) + 1;
    // B loses wpB, gains wpA
    stB.minutes[wpB] -= d;
    stB.minutes[wpA] = (stB.minutes[wpA] || 0) + d;
    stB.counts[wpB] -= 1;
    stB.counts[wpA] = (stB.counts[wpA] || 0) + 1;
  }
  ...
  [8, 6].forEach(function(h) {
    ...
    const { state, targets } = buildState(group);

    // 같은 시간구간 key로 탐색(스왑은 커버리지 유지)
    const byKey = new Map();
    group.forEach(function(w) {
      const st = state.get(w.id);
      st.segs.forEach(function(seg) {
        const key = seg.start + '-' + seg.end;
        if (!byKey.has(key)) byKey.set(key, []);
        byKey.get(key).push({ st: st, seg: seg });
      });
    });

    const MAX_ITERS = 450;
    for (let iter = 0; iter < MAX_ITERS; iter++) {
      // 최악 편차(workplace) 찾기: 범위가 큰 곳을 우선
      let worstWp = null;
      let worstRange = 0;
      WORKPLACES.forEach(function(wp) {
        const vals = group.map(w => (state.get(w.id).minutes[wp] || 0));
        const min = Math.min.apply(null, vals);
        const max = Math.max.apply(null, vals);
        const range = max - min;
        if (range > worstRange) { worstRange = range; worstWp = wp; }
      });
      if (!worstWp || worstRange <= 30) break;

      // worstWp에서 가장 많은 사람/가장 적은 사람
      let hi = null, lo = null;
      group.forEach(function(w) {
        const st = state.get(w.id);
        const v = st.minutes[worstWp] || 0;
        if (!hi || v > hi.v) hi = { st: st, v: v };
        if (!lo || v < lo.v) lo = { st: st, v: v };
      });
      if (!hi || !lo || hi.st.worker.id === lo.st.worker.id) break;

      // hi가 가진 worstWp 세그먼트 중 하나를 골라, 같은 시간대의 다른 근무자와 스왑 시도
      const poolA = hi.st.segs.filter(function(seg) {
        return seg.label === worstWp && (hi.st.counts[worstWp] || 0) >= 2;
      });
      if (!poolA.length) break;

      const segA = pick(poolA);
      const key = segA.start + '-' + segA.end;
      const peers = (byKey.get(key) || []).filter(function(p) { return p.st.worker.id !== hi.st.worker.id; });
      ...
      for (const p of peers) {
        ...
        const segB = p.seg;
        const wpB = segB.label;
        if (wpB === worstWp) continue;

        // lo 쪽이 참여 가능하면 우선
        if (stB.worker.id !== lo.st.worker.id) {
          // lo가 아닌 peer는 개선 폭이 작아질 수 있어 50%만 시도
          if ((rng ? rng.next() : Math.random()) < 0.5) continue;
        }

        if (!canWorkerTake(hi.st.worker, hi.st.counts, segA, wpB)) continue;
        if (!canWorkerTake(stB.worker, stB.counts, segB, worstWp)) continue;

        const delta = deltaForSwap(hi.st, stB, worstWp, wpB, d, targets);
        if (delta < bestDelta) {
          bestDelta = delta;
          best = { stB: stB, segB: segB, wpB: wpB };
        }
      }
      ...
      // apply swap
      const wpA = segA.label;
      const wpB = best.segB.label;
      segA.label = wpB;
      best.segB.label = wpA;
      updateAfterSwap(hi.st, best.stB, wpA, wpB, d);
      fixes.push(h + 'h 균형 스왑: ' + hi.st.worker.name + '↔' + best.stB.worker.name + ' ' + minToTime(segA.start) + '~' + minToTime(segA.end));
    }
  });
...
function workplaceScore(worker, wp, start, end, coverage, rotationCtx) {
  ...
  function chooseWorkplace(worker, start, end, coverage, rotationCtx, rng) {
    ...
    // '하루 동안 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상' 조건: 가능하면 미수행 근무지로 후보를 제한
  ...
  function chooseWorkAssignment(worker, start, gapEnd, coverage, rotationCtx, rng, allocCtx) {
    ...
    // 보조는 다른 일반 근무지가 가능하면 후순위 (단, '하루 동안 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상' 조건으로 보조가 미수행이면 제외하지 않음)
    if (!(worker.requiredWps && worker.requiredWps.has(ASSIST_WP))) {
    ...
    // (단, '하루 동안 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상' 조건을 충족하기 전까지는 비고정 근무지도 선택 가능하도록 강제력을 약화)
    ...
    if (allocCtx && Number.isFinite(allocCtx.blocksLeft) && Number.isFinite(allocCtx.restMinSeg) && Number.isFinite(allocCtx.restMaxSeg)) {
      ...
      if (filteredByBlocks.length) {
        filtered = filteredByBlocks;
      } else {
        // 현재 duration 후보들로는 목표 블록 수를 유지할 수 없음 → 다른 duration을 시도하도록 상위에서 재시도
        return null;
      }
    }
    // '하루 동안 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상' 하드 제약: 가능한 후보가 있으면 미수행 근무지로 후보를 제한
    ...
    // 코어 커버리지(시간대별) 하드 우선: 해당 시간대에 코어 인원이 0이 되지 않게 가능한 후보가 있으면 코어로 제한
    filtered = applyCoreCoverageHardFilter(filtered, coverage, start);
  ...
  function chooseForcedWorkplace(worker, start, end, coverage) {
    ...
    // '하루 동안 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상' 조건: 강제 배정에서도 가능하면 미수행 근무지를 우선
    ...
    // 코어 커버리지(시간대별) 하드 우선: 강제 배정에서도 현재 시간대 코어 결손이 있으면 코어를 우선
    candidates = applyCoreCoverageHardFilterToWps(candidates, coverage, start, end);
...
const presentCoverage = buildPresentCoverage(sortedWorkers);
    ...
    CURRENT_PRESENT_COVERAGE = presentCoverage;
    CURRENT_NON_WORK_COVERAGE = nonWorkCoverage;
    ...
    for (const w of sortedWorkers) {
      ...
      if (w._coreImpossible) {
        w.warnings.unshift('주의: 근무시간(휴식 포함) 대비 코어 근무지(2F 엘베/2F 출차/3F) 1회 이상 조건(최소 ' + w._coreRequiredMin + '분)이 물리적으로 부족할 수 있습니다.');
      }
    }

    CURRENT_MEAL_BREAK_COVERAGE = mealBreakCoverage;

    // v112: 코어 근무지 사전 예약(각 근무자별 1회 이상을 구조적으로 보장)
    preallocateCoreRequiredBlocks(sortedWorkers, coverage, rng);
    ...
    // 하드 조건: 각 근무자는 하루 동안 코어 근무지(2F 엘베/2F 출차/3F)를 최소 1회 이상 수행
    ensureAllWorkplacesAtLeastOnceBySwaps(sortedWorkers, rng);
    // 공평성 보정: 시간대 커버리지는 유지하면서(동일 start-end끼리 스왑) 근무지 편중을 추가 완화
    rebalanceFairWorkplaceDistributionBySwaps(sortedWorkers, rng);
    ...
    // 공평성 점검(스케줄 생성 후 최종 상태 기준)
    const fairnessReport = assessFairWorkplaceDistribution(sortedWorkers);
    if (fairnessReport && fairnessReport.grade === '주의') {
      globalWarnings.unshift('공평성: 일부 근무지 배치 편중이 남아 있습니다. (상세는 공평성 패널에서 확인)');
      // 너무 길지 않게 상위 몇 건만 표시
      (fairnessReport.issues || []).slice(0, 8).forEach(function(msg) {
        globalWarnings.push('공평성: ' + msg);
      });
    } else {
      globalWarnings.unshift('공평성: 근무지 배치가 대체로 균형적입니다.');
    }
    ...
    if (allWpOnceIssues.length) {
      globalWarnings.unshift('중요: "코어 근무지(2F 엘베/2F 출차/3F)는 최소 1회 이상" 조건을 충족하지 못한 근무자가 있습니다.');
    ...
    const result = { workers: sortedWorkers, timeline: timeline, globalCounts: globalCounts, coverageSummary: coverageSummary, meetingCounts: meetingCounts, groupCounts: groupCounts, globalWarnings: globalWarnings, mainPlace60Issues: mainPlace60Issues, generationIndex: generationIndex, seed: rng.seed, fairnessReport: fairnessReport };
