    ...
    for(const e of sorted){
      ...
      for(const needSt of[ST.EXIT,ST.ELEV]){
        ...
        for(let si=0;si<slotsMin.length;si++){
          ...
          const hasRoom=(needSt===ST.EXIT&&cnt.exit<dynMaxAt(ST.EXIT,si,employees,active,schedule))||
                        (needSt===ST.ELEV&&cnt.elev<dynMaxAt(ST.ELEV,si,employees,active,schedule))||
                        (needSt===ST.F3&&cnt.f3<dynMaxAt(ST.F3,si,employees,active,schedule));
          ...
          let swOk=(cnt2.exit<=dynMaxAt(ST.EXIT,si,employees,active,schedule)&&cnt2.elev<=dynMaxAt(ST.ELEV,si,employees,active,schedule)&&cnt2.f3<=dynMaxAt(ST.F3,si,employees,active,schedule));
          // 커버리지 손상 방지
    ...
    for(const nSt of [ST.EXIT,ST.ELEV,ST.F3]){
      ...
      for(let si=0;si<slotsMin.length&&!placed;si++){
        ...
        for(let j=si;j<slotsMin.length&&we-si+adjBefore<coreMaxSlots;j++){
          ...
          if(nSt===ST.EXIT&&cnt.exit>=dynMaxAt(ST.EXIT,j,employees,active,schedule))break;
          if(nSt===ST.ELEV&&cnt.elev>=dynMaxAt(ST.ELEV,j,employees,active,schedule))break;
          if(nSt===ST.F3&&cnt.f3>=dynMaxAt(ST.F3,j,employees,active,schedule))break;
          // 코어 교체 시 커버리지 손상 방지
        ...
        if(!placed&&nSt===ST.F3&&we>si){
          ...
          if(cnt1.f3<dynMaxAt(ST.F3,si,employees,active,schedule)){
            row[si]=ST.F3;
            placed=true; hasIt=true; anyPlacedFCE=true; break outer_a;
          }
        }
        ...
        for(let si=0;si<slotsMin.length&&!placed;si++){
          ...
          for(let j=0;j<swapLenFinal;j++){
            ...
            if(isCore(origSt)){
              ...
              const wouldOver=
                (origSt===ST.EXIT &&cc2.exit >=dynMaxAt(ST.EXIT, si+j,employees,active,schedule))||
                (origSt===ST.ELEV &&cc2.elev >=dynMaxAt(ST.ELEV, si+j,employees,active,schedule))||
                (origSt===ST.F3   &&cc2.f3   >=dynMaxAt(ST.F3,   si+j,employees,active,schedule));
          ...
          for(let j=si;j<si+swapLenFinal&&ok;j++){
            ...
            if(cnt2.exit>dynMaxAt(ST.EXIT,j,employees,active,schedule))ok=false;
            if(cnt2.elev>dynMaxAt(ST.ELEV,j,employees,active,schedule))ok=false;
            if(cnt2.f3>dynMaxAt(ST.F3,j,employees,active,schedule))ok=false;
          }
  ...
  for(let iter=0;iter<50;iter++){
    ...
    for(const d of defs){
      ...
      for(const nSt of needs){
        ...
        if(nSt===ST.EXIT&&cnt.exit>=dynMaxAt(ST.EXIT,d.idx,employees,active,schedule))continue;
        if(nSt===ST.ELEV&&cnt.elev>=dynMaxAt(ST.ELEV,d.idx,employees,active,schedule))continue;
        if(nSt===ST.F3&&cnt.f3>=dynMaxAt(ST.F3,d.idx,employees,active,schedule))continue;
        ...
        const cands=employees.filter(e=>{
          ...
          for(let k=d.idx;k<slotsMin.length;k++){
            ...
            if(nSt===ST.EXIT&&cc.exit>=dynMaxAt(ST.EXIT,k,employees,active,schedule)&&s!==nSt)break;
            if(nSt===ST.ELEV&&cc.elev>=dynMaxAt(ST.ELEV,k,employees,active,schedule)&&s!==nSt)break;
            if(nSt===ST.F3&&cc.f3>=dynMaxAt(ST.F3,k,employees,active,schedule)&&s!==nSt)break;
            fwd++;
        ...
        for(const winner of cands){
          ...
          for(let k=d.idx;k<slotsMin.length&&blockEnd-d.idx+adjBefore<coreMaxSlots;k++){
            ...
            if(nSt===ST.EXIT&&cc2.exit>=dynMaxAt(ST.EXIT,k,employees,active,schedule)&&s!==nSt)break;
            if(nSt===ST.ELEV&&cc2.elev>=dynMaxAt(ST.ELEV,k,employees,active,schedule)&&s!==nSt)break;
            if(nSt===ST.F3&&cc2.f3>=dynMaxAt(ST.F3,k,employees,active,schedule)&&s!==nSt)break;
            blockEnd=k+1;
          ...
          while(blockEnd-blockStart<coreMinSlots&&blockStart>0){
            ...
            if(s!==nSt){
              ...
              if(nSt===ST.EXIT&&ccB.exit>=dynMaxAt(ST.EXIT,k,employees,active,schedule))break;
              if(nSt===ST.ELEV&&ccB.elev>=dynMaxAt(ST.ELEV,k,employees,active,schedule))break;
              if(nSt===ST.F3&&ccB.f3>=dynMaxAt(ST.F3,k,employees,active,schedule))break;
            }
            blockStart=k;
          }
