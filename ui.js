window.UI = (() => {
  ...
  function renderFairnessFold(report) {
    if (!report || !report.groups || !report.groups.length) return '';
    ...
    const groupHtml = report.groups
      .slice()
      .sort(function(a, b) { return Number(b.hours || 0) - Number(a.hours || 0); })
      .map(function(g) {
        const wpStats = g.wpStats || {};
        const entries = Object.keys(wpStats).map(function(wp) {
          const s = wpStats[wp] || {};
          return { wp: wp, range: Number(s.range || 0), min: Number(s.min || 0), max: Number(s.max || 0), avg: Number(s.avg || 0) };
        }).sort(function(a, b) { return b.range - a.range; });

        const top = entries.slice(0, 3).map(function(e) {
          return escapeHtml(shortWorkLabel(e.wp)) + ' ' + escapeHtml(fmtMin(e.min)) + '~' + escapeHtml(fmtMin(e.max)) + ' (범위 ' + escapeHtml(fmtMin(e.range)) + ')';
        }).join('<br>');

        const workers = (g.rows || []).map(function(r) {
          const mins = r.minutes || {};
          const parts = Object.keys(mins).map(function(wp) {
            return shortWorkLabel(wp) + ' ' + fmtMin(mins[wp] || 0);
          }).join(' · ');
          return '<div class="small">' + escapeHtml(r.name || '') + ': ' + escapeHtml(parts) + '</div>';
        }).join('');

        return '<div class="slot-health" style="margin-top:8px">' +
          '<div><strong>' + escapeHtml(String(g.hours)) + '시간</strong> · ' + escapeHtml(String(g.count || 0)) + '명</div>' +
          (top ? ('<div class="muted small" style="margin-top:4px">' + top + '</div>') : '') +
          '<details class="fold-panel inline-fold" style="margin-top:6px"><summary>근무자별 근무지 총합</summary>' +
            '<div class="slot-health">' + workers + '</div>' +
          '</details>' +
        '</div>';
      }).join('');

    const issues = Array.isArray(report.issues) && report.issues.length
      ? ('<div class="warn" style="margin-top:8px">' + report.issues.slice(0, 10).map(escapeHtml).join('<br>') + (report.issues.length > 10 ? '<br>…' : '') + '</div>')
      : '';

    return '<details class="fold-panel inline-fold"><summary>공평성 점검 ' + escapeHtml(grade) + '</summary>' +
      '<div class="slot-health">' + groupHtml + issues + '</div>' +
    '</details>';
  }
  ...
  function renderResult(table, summaryEl, result) {
    ...
    const fairness = result.fairnessReport || null;
    const fairnessGrade = fairness && fairness.grade ? String(fairness.grade) : '';
    ...
    summaryEl.innerHTML =
      ...
      '<div class="counts">' +
        ...
        (fairnessGrade ? ('<span class="pill">공평성 ' + escapeHtml(fairnessGrade) + '</span>') : '') +
