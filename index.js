...
/**
 * dynMaxAt(st, si, employees, active, schedule?) — 슬롯 인덱스 기준 dynMax
 * 슬롯별 실제 활성 인원을 직접 계산해 dynMax에 전달합니다.
 *
 * schedule을 전달하면 CART2F·CART3F 직원을 인원 수에서 제외합니다.
 * CART2F·CART3F는 코어 스테이션이 아니므로 이들을 포함하면 F3 최대
 * 인원이 부풀려져 코어 배정 불균형이 발생하는 버그를 방지합니다.
 */
export function dynMaxAt(st, si, employees, active, schedule = null) {
  let count = 0;
  for (const e of employees) {
    if (!active[e.id][si]) continue;
    if (schedule) {
      const curSt = schedule[e.id]?.[si];
      // 2F카트·3F카트는 코어 배분 대상 외 인원 → 제외
      if (curSt === ST.CART2F || curSt === ST.CART3F) continue;
    }
    count++;
  }
...
export function recolorBlock(ctx, empId, tIdx, targetSt) {
  ...
  const _maxOkAt = (si) => {
    ...
    if (targetSt === ST.EXIT  && cc.exit  >= dynMax(ST.EXIT,  ctx.employees.filter(e=>active[e.id][si]&&schedule[e.id]?.[si]!==ST.CART2F&&schedule[e.id]?.[si]!==ST.CART3F).length))  return false;
    if (targetSt === ST.ELEV  && cc.elev  >= dynMax(ST.ELEV,  ctx.employees.filter(e=>active[e.id][si]&&schedule[e.id]?.[si]!==ST.CART2F&&schedule[e.id]?.[si]!==ST.CART3F).length))  return false;
    if (targetSt === ST.F3    && cc.f3    >= dynMax(ST.F3,    ctx.employees.filter(e=>active[e.id][si]&&schedule[e.id]?.[si]!==ST.CART2F&&schedule[e.id]?.[si]!==ST.CART3F).length))  return false;
    return true;
  };
