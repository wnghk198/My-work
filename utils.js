...
// Bug Fix 8: 여러 직원을 한 번에 추가할 때 renderEmployeeList를 마지막에만 1번 호출
// loadSample / restoreState / doLoad의 루프에서 사용 — N번 호출 시 N번 re-render 방지
export function batchAddEmployees(dataList = []) {
...
export function syncEmpQuickAddUI() {
  ...
  // 출근시간 옵션이 비어있으면 항상 채워넣기
  if (sEl.options.length === 0) {
    for (const t of DEFAULT_START_OPTIONS) {
  ...
  if (g === 'OPEN' || g === 'CLOSE') {
    sEl.disabled = true;
    sEl.value = g === 'OPEN' ? DEFAULTS.OPEN_START : DEFAULTS.CLOSE_START;
  } else {
    sEl.disabled = false;
  }
  // Bug Fix 9: MID 기본 근무시간 6h → 8h (v18 구성에서 7명 중 6h는 이승균 1명뿐)
  hEl.value = '8';
}
...
export function doQuickAdd(setStatus) {
  ...
  const hours = Number($('qaHours')?.value) || 8;  // Bug Fix 9: MID fallback도 8h로 통일
