    ...
    for(const e of employees){
      ...
      for(const needSt of missing){
        ...
        for(const rb of replaceable){
          ...
          const canPlace=(needSt===ST.EXIT&&cnt.exit<dynMaxAt(ST.EXIT,mid,employees,active,schedule))||
            (needSt===ST.ELEV&&cnt.elev<dynMaxAt(ST.ELEV,mid,employees,active,schedule))||
            (needSt===ST.F3&&cnt.f3<dynMaxAt(ST.F3,mid,employees,active,schedule));
          ...
          for(let si=rb.start;si<replEnd;si++){
            ...
            if((needSt===ST.EXIT&&cc2.exit>=dynMaxAt(ST.EXIT,si,employees,active,schedule))||
               (needSt===ST.ELEV&&cc2.elev>=dynMaxAt(ST.ELEV,si,employees,active,schedule))||
               (needSt===ST.F3&&cc2.f3>=dynMaxAt(ST.F3,si,employees,active,schedule))){blockOk=false;break;}
          }
          if(!blockOk)continue;
...
export function repairOverstaffing(ctx){
  ...
  for(let si=0;si<slotsMin.length;si++){
    const activeCount = employees.filter(e => {
      if (!active[e.id][si]) return false;
      const s = schedule[e.id][si];
      // CART2F·CART3F는 코어 배분 대상 외이므로 제외
      ...
      if(i-s>MAX){for(let j=s+MAX;j<i;j++){
        ...
        if(cc.cart3f<CONSTRAINTS.MAX_CART3F){
          ...
          for(const trySt of [ST.F3, ST.EXIT, ST.ELEV]){
            ...
            if(cntSt>=dynMaxAt(trySt,j,employees,active,schedule))continue;
...
export function enforceCloseLateAssignment(ctx){
  ...
  for(let i=lIdx;i<slotsMin.length;i++){
    for(const e of closeE){
      ...
      if(st===ST.EXIT&&c.exit>dynMaxAt(ST.EXIT,i,employees,active,schedule)){
        const alt=[ST.ELEV,ST.F3].find(s=>{const cc=stationCountsAt(schedule,employees,active,i);return(s===ST.ELEV&&cc.elev<dynMaxAt(ST.ELEV,i,employees,active,schedule))||(s===ST.F3&&cc.f3<dynMaxAt(ST.F3,i,employees,active,schedule));});
        schedule[e.id][i]=alt||ST.CART3F;
      }else if(st===ST.ELEV&&c.elev>dynMaxAt(ST.ELEV,i,employees,active,schedule)){
        const alt=[ST.EXIT,ST.F3].find(s=>{const cc=stationCountsAt(schedule,employees,active,i);return(s===ST.EXIT&&cc.exit<dynMaxAt(ST.EXIT,i,employees,active,schedule))||(s===ST.F3&&cc.f3<dynMaxAt(ST.F3,i,employees,active,schedule));});
        schedule[e.id][i]=alt||ST.CART3F;
      }else if(st===ST.F3&&c.f3>dynMaxAt(ST.F3,i,employees,active,schedule)){
        const alt=[ST.EXIT,ST.ELEV].find(s=>{const cc=stationCountsAt(schedule,employees,active,i);return(s===ST.EXIT&&cc.exit<dynMaxAt(ST.EXIT,i,employees,active,schedule))||(s===ST.ELEV&&cc.elev<dynMaxAt(ST.ELEV,i,employees,active,schedule));});
        schedule[e.id][i]=alt||ST.CART3F;
      }
    }
  }
...
export function fillRemainingSlots(ctx){
  ...
  for(const e of employees){const row=schedule[e.id];const actRow=active[e.id];
    ...
    while(i<row.length){
      ...
      if(t.exit>0&&c.exit<dynMaxAt(ST.EXIT,mid,employees,active,schedule)&&canCore(ST.EXIT))fill=ST.EXIT;
      else if(t.elev>0&&c.elev<dynMaxAt(ST.ELEV,mid,employees,active,schedule)&&canCore(ST.ELEV))fill=ST.ELEV;
      else if(t.f3>0&&c.f3<dynMaxAt(ST.F3,mid,employees,active,schedule)&&canCore(ST.F3))fill=ST.F3;
      if(fill){
