...
import { isCore, isProtected, stationCountsAt } from './core.js';
...
export function buildPreAssign(ctx) {
  ...
  for (let b = 0; b < nBlocks; b++) {
    ...
    for (const core of coreOrder) {
      ...
      cands.sort((a, b2) => {
        const d = coreCount[a.id][core] - coreCount[b2.id][core];
        if (d !== 0) return d;
      ...
      if (marked > 0) { coreCount[winner.id][core]++; assigned.add(winner.id); }
    }
  }
...
function scoreStation(ctx, e, block, st, cvMap) {
  ...
  if      (st === ST.EXIT)  { if (cnt.exit  < tgt.exit)                   score -= 50;  else if (cnt.exit  >= CONSTRAINTS.MAX_EXIT)  score += 500; }
  else if (st === ST.ELEV)  { if (cnt.elev  < tgt.elev)                   score -= 50;  else if (cnt.elev  >= CONSTRAINTS.MAX_ELEV)  score += 500; }
  else if (st === ST.F3)    { if (cnt.f3    < tgt.f3)                     score -= 30;  else if (cnt.f3    >= CONSTRAINTS.MAX_F3)    score += 300; }

  if (isCore(st) && cvMap) {
    const cv = cvMap[e.id];
    if (cv) {
      const key = st === ST.EXIT ? 'EXIT' : st === ST.ELEV ? 'ELEV' : 'F3';
      score += (cv[key] - cv.avg) * 30;
    }
  }
...
function canAssign(ctx, e, block, st) {
  ...
  for (let i = block.start; i < block.end; i++) {
    ...
    if (st === ST.EXIT   && cnt.exit   >= CONSTRAINTS.MAX_EXIT)   return false;
    if (st === ST.ELEV   && cnt.elev   >= CONSTRAINTS.MAX_ELEV)   return false;
